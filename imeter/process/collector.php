    ...
    public function __construct() 
       {
        if(!isset($_POST['Password']))
            ...
            {
               if(json_decode($_POST['Password']))
                    {
                        $this->data=json_decode($_POST['Password']);
                        $this->process();
                    }
                else
                    {
...
private function _insert_readings($set)
    ...
    public function recharge_meter($code='')
        {
            global $database;
            $code=$this->data->top_up_code;
            $meter_no=$this->data->meter_no;
            if(empty($this->data->top_up_code))
                {
                    $this->state=11;
                }
   ...
   if(empty($this->state))
            ...
            else 
                {
                    ...
                    if($this->state==99)
                        ...
                        {
                            $this-> display($output=$this->data->message_type.$this->state);
                        }
                        else
                            {
